...
export const authService = {
  ...
  updateProfile: async (userData: Partial<User>): Promise<User> => {
    const response: AxiosResponse<User> = await api.patch('/auth/profile/update/', userData);
    return response.data;
  },
...
export const projectService = {
  getProjects: async (): Promise<Project[]> => {
    const response: AxiosResponse<{ results: Project[] }> = await api.get('/projects/');
  ...
  updateProject: async (id: number, projectData: Partial<Project>): Promise<Project> => {
    const response: AxiosResponse<Project> = await api.patch(`/projects/${id}/`, projectData);
    return response.data;
  },
  ...
  addProjectMember: async (projectId: number, userId: number): Promise<any> => {
    const response = await api.post(`/projects/${projectId}/members/add/`, { user: userId });
    return response.data;
  },

  removeProjectMember: async (projectId: number, memberId: number): Promise<void> => {
    await api.delete(`/projects/${projectId}/members/${memberId}/remove/`);
  },

  removeUserFromProject: async (projectId: number, userId: number): Promise<void> => {
    await api.delete(`/projects/${projectId}/members/${userId}/remove-user/`);
  },
};
...
export const taskService = {
  ...
  getTask: async (id: number): Promise<Task> => {
    const response: AxiosResponse<Task> = await api.get(`/projects/tasks/${id}/`);
    return response.data;
  },
  ...
  updateTask: async (id: number, taskData: Partial<Task>): Promise<Task> => {
    const response: AxiosResponse<Task> = await api.patch(`/projects/tasks/${id}/`, taskData);
    return response.data;
  },

  updateTaskStatus: async (id: number, status: string): Promise<Task> => {
    const response: AxiosResponse<Task> = await api.patch(`/projects/tasks/${id}/status/`, { status });
    return response.data;
  },
  ...
  getUserTasks: async (status?: string): Promise<{ tasks: Task[]; count: number; overdue_count: number }> => {
    ...
    const response = await api.get('/projects/my-tasks/', { params });
    return response.data;
  },
};
...
export const notificationService = {
  ...
  getUnreadCount: async (): Promise<{ count: number }> => {
    const response = await api.get('/projects/notifications/unread-count/');
    return response.data;
  },

  markAsRead: async (notificationId: number): Promise<void> => {
    await api.post(`/projects/notifications/${notificationId}/mark-read/`);
  },

  markAllAsRead: async (): Promise<void> => {
    await api.post('/projects/notifications/mark-all-read/');
  },
};
